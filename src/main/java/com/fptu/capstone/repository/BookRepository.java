...
public interface BookRepository extends JpaRepository<Book, Integer> {
    @Query(value = "select top(18) book_id, status_id, book_name, overall_rating, total_rating, updated_date, description, like_count, enabled,started_date, alias_id, creator_id, image_link from Book order by like_count desc", nativeQuery = true)
    List<Book> findTop10Book();

    @Query(value = "select top(18) book_id, status_id, book_name, overall_rating, total_rating, updated_date, description, like_count, enabled,started_date, alias_id, creator_id, image_link from Book order by started_date desc", nativeQuery = true)
